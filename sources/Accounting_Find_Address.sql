...
IS 
	...
	CURSOR cr_address_email_phone(v_Email VARCHAR2, v_Phone VARCHAR2) IS
		...
		FROM WORKSPACE WS
		JOIN WECO_TOWER1.ADRESSEN_BT A ON A.WORKSPACE$_ID = WS.WORKSPACE$_ID AND A.DELETED_MARK IS NULL
		JOIN WECO_TOWER1.MANDANTEN_BT M ON M.ID = A.MANDANTENID AND M.WORKSPACE$_ID = A.WORKSPACE$_ID AND M.DELETED_MARK IS NULL
	...
	CURSOR cr_address_tax_id(v_Tax_ID VARCHAR2) IS
		...
		FROM WORKSPACE WS
		JOIN WECO_TOWER1.ADRESSEN_BT A ON A.WORKSPACE$_ID = WS.WORKSPACE$_ID AND A.DELETED_MARK IS NULL
		JOIN WECO_TOWER1.MANDANTEN_BT M ON M.WORKSPACE$_ID = WS.WORKSPACE$_ID AND M.DELETED_MARK IS NULL AND A.MANDANTENID = M.ID
	...
	CURSOR cr_address_iban_bic(v_IBAN VARCHAR2, v_SWIFT_BIC VARCHAR2) IS
		...
		JOIN WECO_TOWER1.BANKVERBINDUNG_BT BV 
			ON BV.WORKSPACE$_ID = WS.WORKSPACE$_ID AND BV.DELETED_MARK IS NULL AND A.MANDANTENID = BV.MANDANTENID
	...
	CURSOR cr_Client_address(v_Context_ID NUMBER) IS
		...
		FROM WORKSPACE WS
		JOIN WECO_TOWER1.PROJEKTE_BT P ON P.WORKSPACE$_ID = WS.WORKSPACE$_ID AND P.DELETED_MARK IS NULL
		JOIn WECO_TOWER1.AUFTRAGGEBER_BT AG ON AG.WORKSPACE$_ID = WS.WORKSPACE$_ID AND AG.DELETED_MARK IS NULL AND AG.ID = P.AUFTRAGGEBERID
		JOIN WECO_TOWER1.ADRESSEN_BT A ON A.WORKSPACE$_ID = WS.WORKSPACE$_ID AND A.DELETED_MARK IS NULL AND A.ID = AG.ADRESSENID
		JOIN WECO_TOWER1.MANDANTEN_BT M ON M.WORKSPACE$_ID = WS.WORKSPACE$_ID AND M.DELETED_MARK IS NULL AND A.MANDANTENID = M.ID
